...
void main() {
  ...
  late final GetAllImageUsecase usecase;
  ...
  group("Get All Image Usecase", () {
    ...
    test("Test 1. Success", () async {
      ...
      final result = [
        const FullImage(id: "1si", url: "https://cdn2.thecatapi.com/images/1si.jpg", width: 500, height: 376),
        const FullImage(id: "ct", url: "https://cdn2.thecatapi.com/images/ct.jpg", width: 2992, height: 2992, categories: [Category(id: 5, name: "boxes")]),
        FullImage(id: "z7fJRNeN6", url: "https://cdn2.thecatapi.com/images/z7fJRNeN6.jpg", width: 1024, height: 683, breeds: [Breed(weight: const Weight(imperial: "8 - 17", metric: "4 - 8"), id: "pixi", name: "Pixie-bob", vetstreetUrl: "http://www.vetstreet.com/cats/pixiebob", temperament: "Affectionate, Social, Intelligent, Loyal", origin: "United States", countryCodes: "US", countryCode: "US", description: "Companionable and affectionate, the Pixie-bob wants to be an integral part of the family. The Pixie-Bob’s ability to bond with their humans along with their patient personas make them excellent companions for children.", lifeSpan: "13 - 16", indoor: 0, lap: 1, altNames: '', adaptability: 5, affectionLevel: 5, childFriendly: 4, dogFriendly: 5, energyLevel: 4, grooming: 1, healthIssues: 2, intelligence: 5, sheddingLevel: 3, socialNeeds: 4, strangerFriendly: 4, vocalisation: 1, experimental: 0, hairless: 0, natural: 0, rare: 0, rex: 0, suppressedTail: 1, shortLegs: 0, wikipediaUrl: "https://en.wikipedia.org/wiki/Pixiebob", hypoallergenic: 0, referenceImageId: "z7fJRNeN6")]),
      ];

      /// 2. Action
      when(repository.getAllImages(page, limit)).thenAnswer((_) => Future.value(right(json)));
    ...
    test("Test 2. Fail", () async {
      ...
      /// 2. Action
      when(repository.getAllImages(page, limit)).thenAnswer((_) => Future.value(left(const NetworkFailure("Something went wrong on our end"))));
      final actual = await usecase(param);

      /// 3. Check
      expect(actual, equals(Either<Failure, List<FullImage>>.left(const NetworkFailure("Something went wrong on our end"))));
      verify(repository.getAllImages(page, limit));
    });
  });
}